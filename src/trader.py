...
class Trader:
    ...
    def open_position(self, coin: str, is_buy: bool, size: float,
        ...
        try:
            ...
            if isinstance(result, dict):
                ...
                for st in statuses:
                    ...
                    if "insufficient" in err.lower() or "margin" in err.lower():
                        tg.alert_insufficient_balance(0, notional / leverage, coin)
                        return result
                    if err:
            ...
            if "key" in err_str.lower() or "auth" in err_str.lower():
                tg.alert_api_error(-1, f"API Key 失效或未授權: {err_str}")
            elif "margin" in err_str.lower() or "balance" in err_str.lower():
                tg.alert_insufficient_balance(0, notional / leverage, coin)
            else:
                tg.alert_error("開倉失敗", f"{coin} {side_str}: {err_str}")
            return None

    def close_position(self, coin: str, is_buy: bool, size: float,
                       unrealized_pnl: float = 0.0) -> Optional[dict]:
        sz_dec = self._get_sz_decimals(coin)
        size = _round_size(size, sz_dec)
        if size <= 0:
            return None

        side = "long" if not is_buy else "short"  # 平多方向相反
    ...
    def adjust_position(self, coin: str, current_size: float, target_size: float,
        ...
        if current_side != target_side:
            ...
            self.close_position(coin, is_buy_to_close, current_size, unrealized_pnl)
        ...
        if diff > 0:
            ...
            self.close_position(coin, is_buy_close, abs(diff), unrealized_pnl)
